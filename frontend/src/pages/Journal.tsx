...
const Journal = () => {
  ...
  const { user, discipline } = useSelector((state: RootState) => {
    return {
      user: state.userSlice.user,
      discipline: state.tableDataSlice.selectDiscipline,
    };
  });

  // Данные из стора
  const { tableData, isLoaded } = useSelector(
  ...
  const getSelectTableData = (tableId: number) => {
    ...
    fetchTableData(tableId).then((res) => {
      ...
      filterRows = res.info.filter(
        (tableInfo) => tableInfo.disciplineId === id
      );
      // Если авторизовался студент, то отображаем только его данные
      if (user.role && defineRole(user.role) === "user") {
        filterRows[0].rows = filterRows[0].rows.filter(
          (row) => row.numberRecord == user.id || row.isAdditionalRow
        );
      }
    ...
    <PageWrapper>
      <div className="flex flex-col justify-center min-h-[650px]">
        <div className="flex mb-[50px] justify-center shrink-0">
          ...
          <h2 className="text-[36px]">{discipline}</h2>
        </div>
