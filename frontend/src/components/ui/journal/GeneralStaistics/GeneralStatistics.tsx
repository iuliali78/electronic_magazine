...
const GeneralStatistics = () => {
  ...
  const { attendanceTotal, gradeTotal } = useSelector(
  ...
  useEffect(() => {
    if (data.tableData) {
      const updateingRows = data.tableData.info[0].rows.map((row) => {
        ...
        if (
          Number(attendanceTotal[row.numberRecord]) >= 70 &&
          Number(gradeTotal[row.numberRecord]) >= 4.6
        )
          result = "отлично/зачет";
        else if (Number(gradeTotal[row.numberRecord]) >= 3.6)
          result = "хорошо/зачет";
        ...
        return {
          ...row,
          attendance:
            attendanceTotal[row.numberRecord] &&
            `${attendanceTotal[row.numberRecord]}%`,
